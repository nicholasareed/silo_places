...
define(function(require, exports, module) {
        ...
        this.collection = new AnyModel.AnyCollection([],{
            search_conditions: {
                '$or' : [{
                    ...
                    from_connection_id: that.options.connection_id
                }]
            }
        });
    ...
    SubView.prototype.createDefaultSurfaces = function(){
        ...
        this.loadingSurface = new Surface({
            content: '<i class="icon ion-load-c ion-spin-animation"></i>',
            size: [undefined, 100],
    ...
    SubView.prototype.addOne = function(Model){
        ...
        var userView = new LayoutBuilder({
            surface: {
                surface: new Surface({
                    content: function(){
                        if(Model.get('to_connection_id')){
                            // sent from me
                            return S(Model.get('text'));
                        } else {
                            // received from somebody
                            return '<span>Received '+moment(Model.get('created')).format("ddd, h:mm:ssA")+'</span>' + S(Model.get('text'));
                        }
